...
coldata = read.csv("reservesrs2_srs_assignments.csv")
...
keepC <- coldata1$srs19 %in%  c('SRS1', 'SRS3')
coldata1 <- coldata1[keepC, ]
coldata1$condition <- factor(coldata1$srs19, levels = c("SRS3", "SRS1"))
...
write.csv(as.data.frame(gse1), "SRS19-gsea-SRS1vs3.csv", row.names = FALSE)
...
keepC2 <- coldata2$srs19 %in%  c('SRS2', 'SRS3')
coldata2 <- coldata2[keepC2, ]
coldata2$condition <- factor(coldata2$srs19, levels = c("SRS3", "SRS2"))
...
write.csv(as.data.frame(gse2), "SRS19-gsea-SRS2vs3.csv", row.names = FALSE)
