...
#Create table of immune cell abundance stratified by 19-gene SRS
library(tableone)
library(tableHTML)
listcellvars <- c("bcellsnaive",
                  "bcellsmemory",
                  "plasmacells",
                  "tcellscd8",
                  "tcellscd4naive",
...
tablesrs19cells <- CreateTableOne(vars = listcellvars, 
                                  data = reservesrs4, 
                                  strata = "srs19",
                                  addOverall = TRUE)
tablesrs19cells
summary(tablesrs19cells)

tablesrs19cells <- print(tablesrs19cells, nonnormal = c("bcellsnaive",
                                                        "bcellsmemory",
                                                        "plasmacells",
                                                        "tcellscd8",
                                                        "tcellscd4naive",
...
write_tableHTML(tableHTML(tablesrs19cells), file = 'tablesrs19cells.html')

#Create table of immune cell abundance stratified by 7-gene SRS
listcellvars <- c("bcellsnaive",
                  "bcellsmemory",
                  "plasmacells",
                  "tcellscd8",
...
tablesrs7cells <- CreateTableOne(vars = listcellvars, 
                                 data = reservesrs4, 
                                 strata = "srs7",
                                 addOverall = TRUE)
tablesrs7cells
summary(tablesrs7cells)

tablesrs7cells <- print(tablesrs7cells, nonnormal = c("bcellsnaive",
                                                      "bcellsmemory",
                                                      "plasmacells",
                                                      "tcellscd8",
                                                      "tcellscd4naive",
...
write_tableHTML(tableHTML(tablesrs7cells), file = 'tablesrs7cells.html')