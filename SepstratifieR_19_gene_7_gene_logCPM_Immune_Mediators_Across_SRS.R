...
#Drop if missing mediator data
library(tidyverse)
reservesrs4 <- reservesrs4[!is.na(reservesrs4$logpai1),]

#Create table of biomarkers stratified by 19-gene SRS
library(tableone)
library(tableHTML)

listbiomarkervars <- c("logifn",
                       "logil6",
                       "logil8",
                       "logil10",
                       "logip10",
...
tablesrs19biomarkers <- CreateTableOne(vars = listbiomarkervars, 
                                       data = reservesrs4, 
                                       strata = "srs19",
                                       addOverall = TRUE)
tablesrs19biomarkers
summary(tablesrs19biomarkers)

tablesrs19biomarkers <- print(tablesrs19biomarkers, nonnormal = c("logifn",
                                                                  "logil6",
                                                                  "logil8",
                                                                  "logil10",
                                                                  "logip10",
...
write_tableHTML(tableHTML(tablesrs19biomarkers), file = 'tablesrs19biomarkers.html')

#Create table of biomarkers stratified by 7-gene SRS
listbiomarkervars <- c("logifn",
                       "logil6",
                       "logil8",
                       "logil10",
...
tablesrs7biomarkers <- CreateTableOne(vars = listbiomarkervars, 
                                      data = reservesrs4, 
                                      strata = "srs7",
                                      addOverall = TRUE)
tablesrs7biomarkers
summary(tablesrs7biomarkers)

tablesrs7biomarkers <- print(tablesrs7biomarkers, nonnormal = c("logifn",
                                                                "logil6",
                                                                "logil8",
                                                                "logil10",
                                                                "logip10",
...
write_tableHTML(tableHTML(tablesrs7biomarkers), file = 'tablesrs7biomarkers.html')